...
public class Window_Graph_Tiegel2 : MonoBehaviour {
    ...
    public void ShowGraph(float value, int sekunden, int tiegelFarbe) //Vorher: ShowGraph(List<int> valueList)
    {
        if (changedPos == false)
        {
            if (BBSlot1.GetComponentInChildren<GameObject>().tag.Equals("40SiCold"))
            {
                PanelTiegel.transform.position = new Vector2(-42, -101);
                PanelTiegel.layer = 0;
            }
            else if (BBSlot2.GetComponentInChildren<GameObject>().tag.Equals("40SiCold"))
            {
                PanelTiegel.transform.position = new Vector2(-42, -231);
                PanelTiegel.layer = 0;
            }
            else if (BBSlot3.GetComponentInChildren<GameObject>().tag.Equals("40SiCold"))
            {
                PanelTiegel.transform.position = new Vector2(-42, -387);
                PanelTiegel.layer = 0;
            }
            else if (BBSlot4.GetComponentInChildren<GameObject>().tag.Equals("40SiCold"))
            {
